...
#so essentially a x-y-coordinate system: when I say zero-zero, it is Column Zero and row Zero
var all_pieces = []
var clone_array = []
var current_matches = []
...
# Audio and sound (not music, because it already plays without this)
signal play_audio
# Right now, the above plays when we destroy a piece
# And NOW, it also plays when we press a button
...
func _ready():
	...
	spawn_preset_pieces()
	# Check if this is a board that should have sinkers, and if it is - then spawn sinkers
	if sinkers_in_scene:
...
func move_camera():
	# This is gonna generate a Vector2 where the camera should go.
	# We're gonna use our grid_to_pixel, so we're gonna find out where the
	# camera should be in GRID space, and then we're gonna move it there in
	# PIXEL space
	var new_camera_position = grid_to_pixel(float(width - 1)/2, float(height - 1)/2)
	# 1: Before "Part 63": var new_camera_position = Vector2(grid_to_pixel(width/2-.5, height/2-.5))
	# 2: "Part 63": in 3.1.1: var new_camera_position = Vector2(grid_to_pixel(float(width - 1)/2, float(height - 1)/2))
	# 3: When upgrading from 3.0.6 to 3.1.1: don't cast as Vector2, just send it bare
	emit_signal("place_camera", new_camera_position)
...
func restricted_move(place):
	# Check the licorice (lock) pieces
...
func remove_from_array(array, item): # Function doesn't work - see Part 21 (18:20)
...
func spawn_sinkers(number_to_spawn):
	for i in number_to_spawn: # Always going to be at the top of the board
		# In other words, ROW is always the same - the column will be different each time
...
func _process(delta):
	...
	if can_move:
		...
		# emit some information to the board, and then the board can decide:
		# if it was the booster information, making it a booster state, it can
		# then do with that information whatever it wants to
		extra_input() # This is input when the board is not currently stable

func extra_input():
	# Checks to see if there is a touch on the board
	if Input.is_action_just_pressed("ui_touch"):
		var temp_position = pixel_to_grid(get_global_mouse_position().x, get_global_mouse_position().y)
...
func find_matches(query = false, array = all_pieces):
	...
	if query:
		return false
	get_bombed_pieces()
	get_parent().get_node("destroy_timer").start() 	# This starts the timer
...
func add_to_array(value, array_to_add = current_matches): # Overloads the method
...
func find_bombs():
	# This method finds where are bombs on the board
	...
	if !color_bomb_used:
		for i in current_matches.size():
			...
			var col_matched = 0
			var row_matched = 0
			# Iterate over the current matches (again on the SAME array) to check for column, row, & color
			for j in current_matches.size():
				...
				if this_row == current_row and this_color == current_color:
					row_matched += 1
			# 0 is an adjacent bomb, 1 is a row bomb, 2 is a column bomb and
			# 3 is a color bomb
...
func refill_columns():
	# Check to see how many sinkers are currently on the board
	...
	for i in width:
		for j in height:
			if all_pieces[i][j] == null && !restricted_fill(Vector2(i,j)):
			# COPIED FROM SPAWN PIECES: choose a random number and store it
				var rand = floor(rand_range(0, possible_pieces.size())); 
				var piece = load(possible_pieces[rand]).instance(); 
				var loops = 0;
				while(match_at(i, j, piece.color) && loops < 100):
...
func generate_slime():
	...
	if slime_spaces.size() > 0:
		...
		# Create a tracker variable to ensure we don't enter an infinite loop
		var tracker = 0
		while !slime_made and tracker < 100:
			# Check a random slime
			var random_num = floor(rand_range(0, slime_spaces.size()))
			# The above can also be done via other random integer generators in Godot
			...
			if neighbor != null: 
				print(neighbor)
				# Turn that neighbor tile into a slime tile
				# Remove that piece
				all_pieces[neighbor.x][neighbor.y].queue_free() # Removes it from the board
				# Set it to null
				all_pieces[neighbor.x][neighbor.y] = null
				# Add this new spot to the array of slime tiles
				slime_spaces.append(Vector2(neighbor.x, neighbor.y))
				# Send out a signal to the slime_holder to make a new slime tile
				emit_signal("make_slime", Vector2(neighbor.x, neighbor.y))
				# Set slime_made to be true:
				slime_made = true
			tracker += 1
...
func find_adjacent_pieces(column, row):
	for i in range(-1, 2): # excludes 2, in other words it's only -1, 0, and 1
		for j in range(-1, 2):
...
func destroy_sinkers(): # For all sinkers that hit the bottom of the board
	for i in width:
...
func put_game_in_booster_state(booster_type):
	pass

# BELOW IS INACTIVE -> THIS IS DONE BY GameManager NOW
func booster_input():
	# We check to see if the user is clicking on the board
	if Input.is_action_just_pressed("ui_touch"):
		if current_booster_type == "Color Bomb":
			make_color_bomb(pixel_to_grid(get_global_mouse_position().x, get_global_mouse_position().y))
		elif current_booster_type == "Counter Bomb":
			var tempmcb = get_global_mouse_position()
			if is_in_grid(pixel_to_grid(tempmcb.x, tempmcb.y)):
				make_counter_bomb()
				
# ABOVE IS INACTIVE -> THIS IS DONE BY GameManager NOW
...
func cam_effects():
	...
	# We are gonna call this method when we destroy a piece
...
func _on_lock_holder_remove_lock(place):
	# This will iterate through everything that's inside the lock pieces, but it will iterate backwards
...
func _on_bottom_ui_booster_pressed(booster_type):
	print(booster_type," has been passed to the last method: _on_bottom_ui_booster_pressed")
	put_game_in_booster_state(booster_type)

func _on_GameManager_set_dimensions(new_width, new_height):
	width = new_width
	height = new_height

func _on_GameManager_game_won(winning_score):
	can_move = false
